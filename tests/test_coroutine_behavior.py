...
'''
what to do when coroutines are running still but parent is deleted?
'''
@pytest.mark.asyncio
async def test_one_subscribe():
    d = Dog("bosco", 10)
    d.teach("sit")
    callbacks = dog_callbacks()
    d.subscribe("teach", callbacks.on_teach)
    #d.subscribe("teach", callbacks.async_on_teach)
    d.teach("roll over")
    d.age = 11
    age_subscription = d.subscribe("age", callbacks.on_age_change)
    #age_subscription = d.subscribe("age", callbacks.async_on_age_change)
    d.age += 1

    age_subscription.unsubscribe()
    d.age += 1

    #check while tasks are awaitable
...
def test_one_subscribe_invalid_attribute():
    ...
    callbacks = dog_callbacks()
    try:
        sub = d.subscribe("dog_years", callbacks.on_age_change)
        ...
        assert False
    d.dog_years = 10
    d.subscribe("dog_years", callbacks.on_age_change)
    d.dog_years += 1
    assert callbacks.on_age_change.count == 1
